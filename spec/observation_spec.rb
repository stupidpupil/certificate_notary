...
describe "PerspectivesNotary::Observation#observe_fingerprint" do
  ...
  context 'when the fingerprint matches the most recent observation' do
    ...
    offset = 30
    obs = PerspectivesNotary::Observation.create(service:'service', fingerprint:'fingerprint', start:Time.now-offset, end:Time.now-offset)
    ...
    it 'updates the most recent observation' do
      ...
      expect(PerspectivesNotary::Observation.where(service:'service').last.id).to be obs.id
    end
  end
  ...
  context 'when the fingerprint matches the most recent observation, but the observation is not very recent' do
    ...
    it 'creates a new observation' do
      expect(PerspectivesNotary::Observation.where(service:'service').last.id).not_to be obs.id
    end
  end
  ...
  context "when the fingerprint doesn't match the most recent observation" do
    ...
    offset = 30
    obs = PerspectivesNotary::Observation.create(service:'service', fingerprint:'fungerprunt', start:Time.now-offset, end:Time.now-offset)
    PerspectivesNotary::Observation.observe_fingerprint('service','fingerprint')
